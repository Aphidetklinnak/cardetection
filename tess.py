...
cap = cv2.VideoCapture('carpro/video2.mp4')
...
while True:
    ...
    blob = cv2.dnn.blobFromImage(frame1, 0.007843, (400, 400), 127.5)
    net.setInput(blob)
    ...
    for i in np.arange(0, detections.shape[2]):
        ...
        if confidence > 0.5:
            ...
            box = detections[0, 0, i, 3:7] * np.array([400, 300, 400, 300])
            (startX, startY, endX, endY) = box.astype("int")
            label = "{}: {:.2f}%".format(classes[idx], confidence * 100)
            cv2.rectangle(frame1, (startX, startY), (endX, endY), colors[idx], 2)
            y = startY - 15 if startY - 15 > 15 else startY + 15
            cv2.putText(frame1, label, (startX, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors[idx], 2)
